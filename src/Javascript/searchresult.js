...
function containsNSFWKeywords(query) {
  const nsfwKeywords = ["nsfw", "Nsfw", "adult", "explicit", "inappropriate", "porn", "r34", "rule34", "Hentai", "hentai", "pussy", "loli", "cum", "penis", "p0rn"]; 
...
function displayImageResults(data) {
  ...
  if (data.items && data.items.length > 0) {
    data.items.forEach(item => {
      // Check for NSFW keywords
      if (containsNSFWKeywords(item.title)) {
        // Display a message for NSFW content
        var nsfwMessageDiv = document.createElement('div');
        nsfwMessageDiv.className = 'nsfwMessage';
...
function displayVideoResults(data) {
  ...
  if (data.items && data.items.length > 0) {
    data.items.forEach(item => {
      ...
      searchResultsDiv.appendChild(videoResultDiv);
    });
  } else {
    searchResultsDiv.innerHTML = 'No video results found.';
  }
}
